...
FAQ = [
 ...
 ("Сколько стоит экспертиза?",
  "<p>Стоимость зависит от вида исследования, количества и сложности объектов, объёма материалов и поставленных вопросов. Ориентировочные диапазоны приведены в разделе «Стоимость и сроки». Точную сумму мы называем после изучения задачи и перечня документов — предварительный расчёт бесплатный.</p>"),
 ...
 ("Как передать документы, если я в другом городе?",
  "<p>Материалы можно направить через форму на сайте, по электронной почте или в мессенджере — для предварительной оценки достаточно сканов и фотографий. Оригиналы и объекты исследования передаются позднее, если это необходимо.</p>"),
]
...
<div class="topbar">
  <div class="container topbar__in">
    ...
    <div class="topbar__list">
      ...
      <a href="tel:{ORG['phone_href']}"><b>{ORG['phone_display']}</b></a>
    </div>
  </div>
</div>
...
<header class="header">
  <div class="container header__in">
    <a class="brand" href="{base}index.html" aria-label="{ORG['full']} — на главную">
      <img class="brand__logo" src="{base}assets/img/logo-mark.png" alt="Логотип экспертной организации АЛЬФА" width="120" height="78">
    ...
    <nav class="nav" id="mainnav" aria-label="Основное меню">
      {nav_items}
      <a class="btn btn--primary btn--sm" href="{base}kontakty.html#zayavka">Получить оценку</a>
    </nav>
    <a class="btn btn--primary btn--sm header__cta" href="{base}kontakty.html#zayavka">Получить оценку</a>
  </div>
</header>
"""

def cta_band(base, title="Опишите ситуацию — мы оценим задачу и назовём срок и стоимость",
             text="Предварительная консультация и расчёт — бесплатно. Ответ " + ORG["reply"] + "."):
    return f"""
<section class="cta-band">
  <div class="container cta-band__in">
    ...
    <div class="cta-band__actions">
      <a class="btn btn--primary" href="{base}kontakty.html#zayavka">Получить предварительную оценку</a>
...
<div class="mobile-bar">
  <a class="btn btn--primary" href="{base}kontakty.html#zayavka">Получить оценку</a>
  <a class="btn btn--ghost btn--icon" href="tel:{ORG['phone_href']}" aria-label="Позвонить">{ico(I['phone'],20,'#E8D7AD')}</a>
...
def form(base, dark=False, ident="zayavka", title="Заявка на предварительную оценку",
         subtitle="Опишите ситуацию и приложите документы — эксперт оценит задачу, состав материалов, срок и стоимость."):
...
def build_index():
    ...
    for s in SERVICES:
        ...
        <article class="card">
          ...
          <div class="card__foot">
            ...
            <a class="btn btn--outline btn--sm" href="kontakty.html#zayavka">Оценить ситуацию</a>
          </div>
        </article>"""
    ...
    trust = [
      ...
      ("search","Честная предварительная оценка","Если экспертиза не решает вашу задачу или материалов недостаточно — скажем об этом сразу, до заключения договора."),
    ]
    ...
    price_rows = "".join(f"""<tr><td>{s['title']}</td><td class="num">{s['price']}</td><td class="num">{s['term']}</td></tr>"""
                         for s in SERVICES)
  ...
  <div class="container hero__in">
    <div>
      ...
      <div class="hero__actions">
        <a class="btn btn--primary" href="#zayavka">Получить предварительную оценку</a>
      ...
      <div class="hero__facts">
        ...
        <div class="hero__fact">{ico(I['clock'],20,'#B8944D')}<span><b>Ответ {ORG['reply']}</b>предварительный расчёт бесплатно</span></div>
      </div>
    </div>
    <div class="hero__card">
      {form('', dark=True, ident='hero-form', title='Оценить задачу', subtitle='Ответим ' + ORG['reply'] + '. Предварительный расчёт бесплатный.')}
    </div>
  </div>
</section>
...
{{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{",".join('{"@type":"Question","name":%s,"acceptedAnswer":{"@type":"Answer","text":%s}}' % (_json(q), _json(re.sub("<[^>]+>","",a))) for q,a in FAQ)}]}}
    ...
    page("index.html", "Экспертная организация «АЛЬФА» — судебные и внесудебные экспертизы в Нижнем Новгороде",
         "Строительно-техническая, автотехническая, землеустроительная, товароведческая и финансово-экономическая экспертиза. Рецензии на заключения. Предварительная оценка задачи бесплатно.",
  ...
  <div class="container split" style="align-items:flex-start;gap:48px">
    <div>
      ...
      <div class="note" style="margin-top:24px">
        Для <strong>предварительной оценки</strong> достаточно сканов или фотографий документов. Оригиналы и объекты исследования передаются после заключения договора.
      </div>
    ...
    <aside>
      <div class="card" style="position:sticky;top:100px">
        ...
        <p style="font-size:14.5px">Итоговая стоимость зависит от количества объектов, числа вопросов и необходимости выезда. Предварительный расчёт — бесплатно.</p>
        <a class="btn btn--primary btn--wide" href="../kontakty.html#zayavka">Оценить ситуацию</a>
...
# ════════════════════════════════════════════════════════════════
def build_prices():
    rows = "".join(f"<tr><td>{s['title']}</td><td>{s['short']}</td><td class='num'>{s['price']}</td><td class='num'>{s['term']}</td></tr>" for s in SERVICES)
  ...
  <div class="container">
    <div class="table-wrap">
      <table class="price">
        ...
        <tbody>{rows}
          <tr><td>Информационное письмо в суд</td><td>Срок, стоимость, сведения об экспертах</td><td class="num">бесплатно</td><td class="num">1 рабочий день</td></tr>
          <tr><td>Предварительная оценка задачи</td><td>Анализ ситуации и состава материалов</td><td class="num">бесплатно</td><td class="num">{ORG['reply']}</td></tr>
          <tr><td>Участие эксперта в судебном заседании</td><td>Пояснения по заключению, ответы на вопросы</td><td class="num">от 8 000 ₽</td><td class="num">по вызову суда</td></tr>
          <tr><td>Выезд за пределы области</td><td>Транспорт и проживание</td><td class="num">по согласованию</td><td class="num">—</td></tr>
        </tbody>
      </table>
    </div>
    ...
    page("ceny.html","Стоимость и сроки экспертизы — «АЛЬФА», Нижний Новгород",
         "Ориентировочная стоимость и сроки проведения судебных и внесудебных экспертиз. Предварительный расчёт и письмо в суд — бесплатно.",
...
<section class="section">
  <div class="container" style="max-width:860px">
    ...
    <ul class="checklist">
      ...
      <li>предварительная оценка задачи, срока и стоимости услуг;</li>
...
<section class="section">
  <div class="container" style="max-width:860px">
    ...
    <p>Рассмотрение обращения, подготовка ответа и предварительного расчёта, заключение и исполнение договора на оказание экспертных услуг.</p>
